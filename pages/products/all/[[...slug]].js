...
const Products = (props) => {
    ...
    const { search, sort, max_price, min_price } = router.query;
...
export const getStaticProps = async (context) => {
    ...
    // // Fetch menu
    let res1 = await fetch(apiUrl + 'items/menu?fields=title,url,position&filter[status]=published&sort=sort');
    res1 = await res1.json();
    const menu = res1.data;
    const navbarMenu = [
    ...
    // // Fetch products
    // res = await fetch(apiUrl + 'items/products?fields=image,title,price,discount&filter[status]=published&sort=created_on&limit=' + limit + '&q=' + search);
    // res = await res.json();
